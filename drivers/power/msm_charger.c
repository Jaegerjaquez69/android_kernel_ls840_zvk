...
#ifdef CONFIG_MACH_LGE_120_BOARD //[Preserving Fake Battery Mode] sujin.shin@lge.com
...
#define DEFAULT_BATT_MIN_V(x)   (3500 - (x*100))
#define DEFAULT_BATT_MAX_V(x)   (4350 - (x*150))
#else
...
// [TEMP_CONTROL - Preserving Fake battery mode setting] sujin.shin@lge.com
#define FAKE_BATT_ENABLED 1
#define FAKE_BATT_DISABLED 0
//[Fake battery mode disable Temperature - To prevent phone from being damaged in high temperature environment(over 68celcius)] sujin.shin@lge.com
...
//20111227 ws.yang@lge.com add to set charging current when lcd on or off [S]
extern int lm3530_backlight_status(void);
//20111227 ws.yang@lge.com add to set charging current when lcd on or off [E]
#endif
...
/* [LGE_UPDATE_S kyungho.kong@lge.com] */
  ...
  struct alarm alarm;
#endif
// [OVERHEAT_FAKE:START] sujin.shin@lge.com
	int lcd_on_charge_curr_status;
	int capacity;
	int chg_current_set;
// [OVERHEAT_FAKE:END] sujin.shin@lge.com
...
int is_batt_status_charging(void)
{
...
static enum power_supply_property msm_batt_power_props[] = {
	...
	POWER_SUPPLY_PROP_CHARGING_CURRENT_SET, // [OVERHEAT_FAKE] sujin.shin@lge.com
...
*/		
  ...
  { 
    /* [LGE_UPDATE_ S : For Fuel Gauge Tuning] - 20120412*/
    cur_soc_val = get_prop_batt_capacity();
    orig_cur_soc_val = cur_soc_val;
   ...
   fuel_percent = val->intval;
...
#ifdef CONFIG_LGE_CHARGER_TEMP_SCENARIO
	case POWER_SUPPLY_PROP_TEMP: // temp�� �µ� �� 
#ifdef CONFIG_MACH_LGE_120_BOARD  
	...
	case POWER_SUPPLY_PROP_BATTERY_TEMP_ADC: // �µ� ADC ��. 
#ifdef CONFIG_MACH_LGE_120_BOARD
    if((pseudo_batt_info.mode == 1)&& (msm_chg.temperature >= FAKE_BATT_DISABLE_TEMP))
...
static struct power_supply msm_psy_batt = {
	...
	.set_property = msm_batt_power_set_property, //20110207 ws.yang@lge.com
	.property_is_writeable = msm_batt_power_property_is_writeable, //20110207 ws.yang@lge.com
};
...
extern int testmode_charging_onoff;
#endif

static void handle_battery_inserted(void)
{
#if defined(CONFIG_MACH_LGE_C1_BOARD_SPR)
	if(testmode_charging_onoff)
#endif	
	{
...
#ifdef CONFIG_LGE_CHARGER_TEMP_SCENARIO
int g_temp_adc = 1000;
static int chg_is_battery_too_hot_or_too_cold(int temp_adc, int batt_level, int curr_set_status) // [OVERHEAT_FAKE] sujin.shin@lge.com
{
    int chg_batt_temp;
    int rtnValue = 0;
/* kiwone.seo@lge.com 2011-0609 for show charging ani.although charging is stopped : charging scenario*/
    ...
    {
#ifdef CONFIG_LGE_SPRINT_TEMP_SCENARIO
...
/* kiwone.seo@lge.com 2011-0609 for show charging ani.although charging is stopped : charging scenario*/
#ifdef CONFIG_LGE_SPRINT_TEMP_SCENARIO
            if(chg_batt_temp == CHG_BATT_TEMP_UNDER_M10 || chg_batt_temp == CHG_BATT_TEMP_M10_M5 ||(chg_batt_temp == CHG_BATT_TEMP_46_55 && batt_level > 4000))
#else
...
#ifdef CONFIG_LGE_SPRINT_TEMP_SCENARIO
          else if(chg_batt_temp == CHG_BATT_TEMP_M4_41 || chg_batt_temp == CHG_BATT_TEMP_42_45 )
#else
...
#ifdef CONFIG_LGE_SPRINT_TEMP_SCENARIO
        if(chg_batt_temp == CHG_BATT_TEMP_M4_41)
#else
...
#ifdef CONFIG_LGE_SPRINT_TEMP_SCENARIO
            if(chg_batt_temp == CHG_BATT_TEMP_UNDER_M10 || chg_batt_temp == CHG_BATT_TEMP_M10_M5)
#else
			if(chg_batt_temp == CHG_BATT_TEMP_UNDER_M10 ||(chg_batt_temp == CHG_BATT_TEMP_46_55 && batt_level > 4000))
#endif
			{
...
//sujin.shin [TEMP_CONTROL]
	...
	msm_chg.capacity = get_prop_batt_capacity();  // [OVERHEAT_FAKE] sujin.shin@lge.com
	if (msm_chg.current_chg_priv
		...
		stop_charging = chg_is_battery_too_hot_or_too_cold(temp_adc,battery_level,msm_chg.lcd_on_charge_curr_status); // // [OVERHEAT_FAKE] sujin.shin@lge.com
...
case 4 : charging is started in normal case and then if temp is very hot/cold, we should call pm_chg_auto_disable(1) */
		...
		{
			pm_chg_auto_disable(1);
			pr_err("msm-charger stop charging by scenario\n");
...
/* kiwone.seo@lge.com 2011-0609 for show charging ani.although charging is stopped : charging scenario*/
		...
		{
			pm_chg_auto_disable(0);
			pr_err("msm-charger start charging by scenario\n");
			last_stop_charging = stop_charging;
/* kiwone.seo@lge.com 2011-0609 for show charging ani.although charging is stopped : charging scenario*/
			pseudo_ui_charging = 0;
#if 1 // after stop charging, charging is working when TA re-inserted to inform UI
				   ...
				   *hw_chg_removed, int new_state)
{
	...
	msm_chg.lcd_on_charge_curr_status = 0; // [OVERHEAT_FAKE] sujin.shin@lge.com
...
#ifdef LGE_DEBUG
		printk("%s: CHG_BATT_BEGIN_FAST_CHARGING\n", __func__);
#endif
...
#ifdef LGE_DEBUG
		printk("%s: CHG_BATT_NEEDS_RECHARGING\n", __func__);
#endif
		msm_chg.batt_status = BATT_STATUS_DISCHARGING;
		handle_battery_inserted();
		priv = msm_chg.current_chg_priv;
...
#ifdef LGE_DEBUG
		printk("%s: CHG_BATT_TEMP_OUTOFRANGE\n", __func__);
#endif
		/* the batt_temp out of range can trigger
		 * when the battery is absent */
		 //20110126 ws.yang@lge.com add to charging in low temp when fake mode
		if (pseudo_batt_info.mode == 1) {			
...
#ifdef LGE_DEBUG
		printk("%s: CHG_BATT_TEMP_INRANGE\n", __func__);
#endif
		if (msm_chg.batt_status != BATT_STATUS_TEMPERATURE_OUT_OF_RANGE)
			break;
		msm_chg.batt_status = BATT_STATUS_ID_INVALID;
...
#ifdef LGE_DEBUG
		printk("%s: CHG_BATT_STATUS_CHANGE\n", __func__);
#endif
...
/* [LGE_UPDAET_S : for Fuel Gauge BUG] */
#ifdef CONFIG_LGE_PM_BATTERY_ALARM
	case CHG_BATT_REFRESH_EVENT:
#ifdef LGE_DEBUG
		printk("%s: CHG_BATT_REFRESH_EVENT\n", __func__);
#endif
		break;
#endif
/* [LGE_UPDAET_S : for Fuel Gauge BUG] */
...
static ssize_t at_chg_status_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
  ...
  if(!strncmp(string, "0", 1))
  {
    ...
    /* 20111012 hoyeon.jang@lge.com, check NULL of msm_chg.current_chg_priv {++*/
    if (msm_chg.current_chg_priv != NULL)
    {
      priv = msm_chg.current_chg_priv;
  	  ret = priv->hw_chg->stop_charging(priv->hw_chg);
...
void testmode_discharging_mode_test(void)
{
 ...
 /* Stop Charging */
    ...
    {
#ifdef LGE_DEBUG
      printk(KERN_DEBUG "############ [chg_status_store] STOP CHARGING #####################\n");    
#endif
     ...
     msm_chg.batt_status = BATT_STATUS_DISCHARGING;
	  ...
	  msm_charger_notify_event(usb_hw_chg_priv->hw_chg, CHG_DONE_EVENT);
#else
      msm_chg.batt_status = BATT_STATUS_ABSENT;
      b_is_testmode_cmd_on = false;
#endif
...
void testmode_wq_charging_mode_test(struct work_struct *work)
{
  int ret;
...
EXPORT_SYMBOL(testmode_wq_charging_mode_test);
...
static int __devinit msm_charger_probe(struct platform_device *pdev)
{
  ...
  int err;
    ...
    if (msm_chg.max_voltage == 0)
    	msm_chg.max_voltage = DEFAULT_BATT_MAX_V(lge_battery_info);
    if (msm_chg.min_voltage == 0)
    	msm_chg.min_voltage = DEFAULT_BATT_MIN_V(lge_battery_info);
#else
	if (msm_chg.max_voltage == 0)
...
static int msm_charger_resume(struct device *dev)
{
	...
	queue_delayed_work(msm_chg.event_wq_thread,
				&msm_chg.update_heartbeat_work,
			      round_jiffies_relative(msecs_to_jiffies
						     (msm_chg.update_time)));
	msm_chg.chg_current_set = 0; // [OVERHEAT_FAKE] sujin.shin@lge.com
	mutex_lock(&msm_chg.status_lock);
	handle_battery_inserted();

#ifdef CONFIG_LGE_PM_BATTERY_ALARM
	/* platform-bsp@lge.com (20111213) : for update uevent */

	msm_charger_notify_event(NULL, CHG_BATT_REFRESH_EVENT); /* [LGE_UPDATE: for Fuel Gauge BUG] */

	soc = max17040_get_battery_capacity_percent();
#if 0 /* platform-bsp@lge.com (20111213) : for E1 */
